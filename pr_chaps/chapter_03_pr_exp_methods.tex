...
The intrinsic resolution of a semiconductor detector stems from statistical fluctuations in how many electrons and holes are collected at the terminals. As these arise from simple counting statistics one can estimate the resolution of the detectors.
\begin{equation}
\label{eqn:hpge-res-est} 
\%Res \propto N^{-\frac{-1}{2}}
\end{equation}
Where N is the number of charge carriers produced and is approximately $E_{\gamma}/\epsilon$ where $\epsilon$ is the average energy to make an electron hole pair ($2.96eV$ for Ge). In a more careful examination we define the detector resolution to be the Full Width at Half Maximum (FWHM) of a peak. The resolution then becomes:
\begin{equation}
\label{eqn:hpge-res} 
FWHM(E) = 2\sqrt{2 Ln(2)}\frac{E_{\gamma{}}F}{\sqrt{E_{\gamma{}}/\epsilon{}}}\\
