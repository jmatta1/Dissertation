...
For the accurate and efficient detection of \gr{}s one must optimze two
The resolution of the full energy peak in a spectrum produced by one of these detectors is, absent other factors, the error in the counting of the secondary elections. For standard counting statistics the error associated with a value is simply the square root of the value in question, giving that the error in the value (or resolution in the peak) is proportional to $\%{}err\thicksim{}1/\sqrt{N}$ where $N$ is the value. Thus we see that to achieve high resolution we must increase N as much as possible.
